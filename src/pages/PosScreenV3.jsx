...
import WeightComposer from "../components/WeightComposer";
import { db } from "../data/db";
import { getT, localName } from "../i18n";
import { money } from "../lib/format";
import { formatWeight, isWeightedItem } from "../lib/unitEngine";
...
export default function PosScreenV3({
  lang,
  notify,
  initialTable,
  initialOrder,
  ...
  const [weightedItem, setWeightedItem] = useState(null);
  ...
  const add = (item) => {
    if (isWeightedItem(item)) return setWeightedItem(item);
    setCart((rows) =>
        ...
        ? rows.map((line) =>
            line.itemId === item.id
              ? {
                  ...line,
                  qty: line.qty + 1,
                  lineTotal: line.price * (line.qty + 1),
                }
              : line
          )
        : [
  ...
  const addWeight = ({ kilograms, amount }) => {
        ...
        .map((line) =>
          line.itemId === id
            ? {
                ...line,
                qty: line.qty + delta,
                lineTotal: line.price * (line.qty + delta),
              }
            : line
        )
  ...
  const removeLine = (id) =>
    setCart((rows) => rows.filter((line) => line.itemId !== id));
  ...
  const assignmentName =
    options.find((row) => row.id === Number(assignment))?.name || "";
  ...
  const submit = async (settle = false) => {
    ...
    try {
      if (orderType === "dinein" && !settle) {
        ...
        let id = openOrderId;
        if (id)
          await db.orders.update(id, { ...payload, createdAt: undefined });
    ...
    <>
      <section className="pos-page">
        ...
        <aside className="order-pane">
            ...
            {cart.length ? (
              cart.map((line) => (
                <div className="cart-line" key={line.itemId}>
                  ...
                  <div className="line-name">
                    ...
                    <small>
                      {line.saleMode === "weight"
                        ? `${formatWeight(line.qty)} · ${money(line.price)}/kg`
                        : money(line.price)}
                    </small>
                  </div>
                  {line.saleMode === "weight" ? (
                  ...
                  <strong>
                    {money(line.lineTotal ?? line.price * line.qty)}
                  </strong>
                </div>
              ))
            ) : (
          ...
          >
            <span>
              {processing && <i className="button-spinner" />}
              {orderType === "dinein"
                ? openOrderId
                  ? t("updateTable")
                  : t("sendTable")
